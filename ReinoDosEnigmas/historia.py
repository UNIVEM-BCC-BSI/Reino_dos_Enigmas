...
def historia():
    ...
    def escolhaFloresta():
        print(f'\n\n Entrando na floresta, {playerClass.name} vai andando devagar e com muito cuidado até se deparar com uma Cobra, ele começa a correr com todo o seu folego e consegue encontrar uma caverna para se esconder.')
        while True:
            textoAnimado('\nQual será sua escolha?\n\n1 - Seguir correndo pela floresta \n2 - Entrar na caverna para se esconder\n\nEscolha: ', 0.02)
            escolhaParte2 = int(input())
            if escolhaParte2 == 1:
                print(f"\n\nSeguindo pela floresta {playerClass.name} consegue aos poucos ir percebendo que ele estava chegando perto de uma civilização, até encontrar uma segunda trilha bem na sua frente, e para a sua surpresa ela aparentava ser bem utilizada.\nSeguindo ela o jogador em pouco tempo já se deparava com os enormes muros de uma grande cidade. Entrando nela, ele logo encontrou diversos tipos de comerciantes, guardas da cidade e grande parte da população reunida em um grande centro com uma enorme praça. \nDesesperado e com medo {playerClass.name} vai atrás de ajuda conversando com um vendedor o jogador acaba descobrindo que ele é um fazendeiro que morava perto da cidade onde ele se encontrava.")
                break
                ...
                while True:
                    print("\n\n Ciclo das rochas é um fenômeno natural e contínuo responsável pela renovação e transformação da litosfera terrestre com o passar tempo através de fenômenos, como")
                    resp = str(input())
                    if resp.lower() == 'a':
                        playerClass.daPontoGeo
                        print(playerClass.dinheiro)
                        playerClass.daDinheiro(10)
                        print(playerClass.dinheiro)
